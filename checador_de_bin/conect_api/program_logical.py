...
class Requisicao(object):
    ...
    def func_valida(self):
        ...
        if len(self.__bin) < 4: # PARA MEXER COM WIDGETS DE TEXTO TEEM QUE ESTA state:NORMAL
    ...
    def dados(self,bin):
        try:
            self.__widget2.delete(1.0,tk.END)
            self.resque = requests.get(f'https://api-center.000webhostapp.com/api/apistore_binchecker.php?bin={bin}')
